...
def evidence(root, arch="amd64", **changes):
    row = {"source": SHA, "image": release.IMAGE + "@sha256:" + ("b" if arch == "amd64" else "c") * 64,
           "arch": arch, "verified": True, "checks": "compose-oauth-http-restart-revocation", **changes}
...
@pytest.mark.parametrize("changes", [
    {"source": "d" * 40}, {"verified": False}, {"checks": "build-only"},
    {"image": "attacker/image@sha256:" + "a" * 64}, {"image": release.IMAGE + ":latest"},
])
...
def test_only_latest_is_promoted_without_rebuilding(tmp_path, monkeypatch):
    ...
    assert calls[0] == ["docker", "buildx", "imagetools", "create", "--tag", release.IMAGE + ":latest",
                        release.IMAGE + "@sha256:" + "b" * 64, release.IMAGE + "@sha256:" + "c" * 64]
