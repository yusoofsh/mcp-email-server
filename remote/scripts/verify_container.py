"""Exercise the exact image with canonical Compose, without real mail.

Uses a random test-only Compose project and its own temporary credential/volume.
The HTTP client models HTTPS termination with the configured Host/Origin.
...
def verify(image: str, expected_version: str | None, source: str | None, report: Path | None):
    ...
    with tempfile.TemporaryDirectory(prefix=project) as temporary:
        ...
        env = {
            **os.environ,
            "MCP_IMAGE": image, "MCP_BIND_PORT": "0", "MCP_PUBLIC_URL": PUBLIC,
            "MCP_AUTH_USERNAME": "test-operator", "MCP_AUTH_REDIRECT_URIS": CALLBACK,
        ...
        try:
            ...
            client.wait()
            if expected_version:
                actual = run(compose + ["exec", "-T", "email-mcp", "/opt/email/bin/mcp-email-server", "--version"], env=env)
                assert actual == expected_version
            run(compose + ["exec", "-T", "email-mcp", "/opt/remote/bin/email-mcp-remote", "smoke-engine"], env=env)
            ...
            assert client.request("/mcp", method="POST", payload={}, headers={"Authorization": "Basic " + basic})[0] == 401
            ...
            status, _, body = client.request("/register", method="POST", payload={
                "redirect_uris": [CALLBACK], "client_name": "Synthetic container test",
            ...
            status, headers, _ = client.request("/authorize?" + urllib.parse.urlencode({
                "client_id": client_id, "redirect_uri": CALLBACK, "response_type": "code", "scope": SCOPE,
                "state": "container-test", "code_challenge": challenge, "code_challenge_method": "S256",
                "resource": PUBLIC + "/mcp",
            }))
            ...
            login_headers = {"Origin": PUBLIC, "Cookie": "; ".join(f"{key}={value.value}" for key, value in cookie.items())}
            form = {"ticket": ticket, "csrf": csrf, "username": "test-operator", "password": password, "decision": "approve"}
            assert client.request("/login", method="POST", form={**form, "csrf": "invalid"}, headers=login_headers)[0] == 403
            assert client.request("/login", method="POST", form={**form, "password": "wrong"}, headers=login_headers)[0] == 401
            ...
            grant = {"grant_type": "authorization_code", "client_id": client_id, "code": params["code"][0],
                     "redirect_uri": CALLBACK, "code_verifier": verifier, "resource": PUBLIC + "/mcp"}
            assert client.request("/token", method="POST", form={**grant, "resource": "https://attacker.example/mcp"})[0] == 400
            ...
            def rpc(method, parameters=None):
                status, _, body = client.request("/mcp", method="POST", payload={
                    "jsonrpc": "2.0", "id": 1, "method": method, "params": parameters or {},
            ...
            client.wait()
            rpc("tools/list")
            status, _, body = client.request("/token", method="POST", form={
                "grant_type": "refresh_token", "client_id": client_id, "refresh_token": pair["refresh_token"],
                "resource": PUBLIC + "/mcp",
            })
            assert status == 200
            ...
            pair = refreshed
            rpc("ping")
            assert client.request("/revoke", method="POST", form={"client_id": client_id,
                "token": pair["refresh_token"], "token_type_hint": "refresh_token"})[0] == 200
            assert client.request("/mcp", method="POST", payload={}, headers={"Authorization": "Bearer " + pair["access_token"]})[0] == 401
        finally:
    ...
    if report:
        ...
        report.write_text(json.dumps({"source": source, "image": image, "arch": metadata["Architecture"],
                                      "verified": True, "checks": "compose-oauth-http-restart-revocation"}) + "\n")
